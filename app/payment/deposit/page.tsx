...
export default function Meeting({ isSidebarOpen, toggleSidebar, isCollapsed, toggleCollapse }: MeetingProps) {
  const [orderId, setOrderId] = useState<string>("");
  const [copied, setCopied] = useState<boolean>(false);
  const [paymentStatus, setPaymentStatus] = useState<"pending" | "success" | "failed">("pending");
  const [userData, setUserData] = useState<PaymentDetails | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchPaymentStatus = async () => {
      try {
        ...
        const paymentDetails: PaymentDetails = JSON.parse(decryptedDetails);
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <button
              onClick={() => router.back() || router.push("/user")}
